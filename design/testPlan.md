...
|---|---|---|---|
| **TP-A00** | §4.4.4 | **Run this before TP-A01.** Verify the rig before trusting any reading from it: R5 continuity to 0 V, R6 to the summing node, R10 **absent**. Then unplug both sensor outputs from R8/R9 and measure PC4. | **≤ 5 counts (≈0.016 V).** *(Executed 2026-08-08 — **FOUND TWO FAULTS**: R10 still fitted and R5 floating. Every measurement taken before the fix was of a circuit that was neither the schematic nor a divider, and all of it was discarded. This row earned its place on first use.)* |
...
| **TP-A02** | §4.4.3 | With both sensors connected and powered, measure PC4 in all three states: neither active, each one active separately, both active. | Within **±5 %** of 0.310 / 1.481 / 2.300 V. *(Executed 2026-08-08 on the corrected rig: 0.310 / 1.477 / 1.485 / 2.300 — pass, model agreement ≤1 mV.)* |
| **TP-A03** | §4.4.4 | Sweep the sensor supply 20.4 → 27.6 V. Record PC4 for one-active and both-active at each end. | One active ≤ 529 counts and both active ≥ 606 counts, i.e. the 567 threshold is not crossed from either side. |
