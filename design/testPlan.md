...
|---|---|---|---|
| **TP-A00** | §4.4.4 | **Run this before TP-A01.** Unplug *both* sensor output leads from R8/R9, leaving the sensors powered and the summing network otherwise intact. Measure PC4. | **≤ 5 counts (≈0.016 V).** The node should sit at ground through R5 alone. If it still reads anything like 0.329 V, the 35 µA is **the breadboard, not the sensors** — 24 V / 35 µA is 686 kΩ, which a contaminated board can present between adjacent rows. That would change the §4.4.4 conclusion completely and make TP-A01 moot. |
| **TP-A01** | TDS §6, §4.4.4 | **THE BLOCKING TEST — only meaningful once TP-A00 passes.** Disconnect sensor B. With sensor A powered and inactive, record PC4 at room temperature. Warm the sensor body by ~20 °C (hot air, low; measure the case). Record PC4 every 5 °C. | PC4 rises by **less than 30 %** over the sweep → the 35 µA is an internal bleeder, §4.4 stands. A steep climb → junction leakage, and §4.4.2's values need re-deriving with a smaller `Rs` and a lighter load. **Record the curve, not just the verdict.** |
| **TP-A02** | §4.4.3 | With both sensors connected and powered, measure PC4 in all three states: neither active, each one active separately, both active. | Within **±5 %** of 0.329 / 1.502 / 2.320 V. *(Executed 2026-08-08: 0.329 / 1.505 / 1.498 / 2.320 — pass.)* |
| **TP-A03** | §4.4.4 | Sweep the sensor supply 20.4 → 27.6 V. Record PC4 for one-active and both-active at each end. | One active ≤ 536 counts and both active ≥ 612 counts, i.e. the 574 threshold is not crossed from either side. |
