...
| [`../docs/modbusInterfaceContractSpecification.md`](../docs/modbusInterfaceContractSpecification.md) | **Interface contract for the client** — the greenhouse controller's view: where each value lives, what it means and does *not* mean, how to teach, which bits are alarms and which are health. Organised by purpose, every statement traced to a TDS requirement. §8 sizes the configuration to the target window (1.5 m in ≈2 min) and corrects the study's FR-WP20 threshold, which would reject that window's own motion. | **v1.1.** Derived from TDS v0.7; the TDS stays normative. Not a design input — a design *output* for the other side of the bus |
